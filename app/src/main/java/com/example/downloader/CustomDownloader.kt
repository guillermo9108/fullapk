...
class CustomDownloader(
    ...
    companion object {
        // Shared globally across all CustomDownloader instances to enforce one download at a time
    ...
    suspend fun startDownload(downloadUrl: String, customFilename: String? = null) {
        ...
        // Initial insert as QUEUED so the user sees it in their queue list immediately!
        ...
        // Use a Mutex to serialize downloads. Subsequent requests will wait in a pending state.
        downloadMutex.withLock {
            // Check if the item still exists in the DB (i.e. has not been deleted or cancelled by user while queued)
            val currentCheck = downloadDao.getDownloadById(id)
            if (currentCheck == null) {
                Log.d("CustomDownloader", "Download $id was cancelled/deleted while in queue.")
                return@withLock
            }

            // Transition status from QUEUED to DOWNLOADING
            val transitionItem = DownloadItem(
                id = id,
            ...
            downloadDao.insertDownload(transitionItem)
                ...
                try {
                    ...
                    while (inputStream.read(buffer).also { bytesRead = it } != -1) {
                        ...
                        if (now - lastProgressUpdate > 300) {
                            // Check if the item was deleted from DB (cancelled) by user during download (every 1.5s)
                            if (now - lastDatabaseCheck > 1500) {
                                lastDatabaseCheck = now
                                if (downloadDao.getDownloadById(id) == null) {
                                    Log.d("CustomDownloader", "Download $id was cancelled/deleted from UI during execution.")
                                    outputStream.close()
                                    inputStream.close()
                    ...
                    // Only mark as failed in database if the record was not deleted completely from UX
                    val currentCheck = downloadDao.getDownloadById(id)
                    if (currentCheck != null) {
